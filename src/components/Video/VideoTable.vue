<template>
  <div>
    <h4 style="margin-bottom: 10px;">{{video_table.typeName}}</h4>
    <table class="am-table am-table-hover" :class="{ 'b3': videos_is_live, 'b1': !videos_is_live}">
      ...
      <tbody v-if="videos_is_live">
        <tr v-for="video in video_table.videos" @click="gotoVideo(video.videoId)" :class="{cur: current_id==video.videoId}">
      ...
      <tbody v-else>
        <tr v-for="video in video_table.videos" @click="gotoVideo(video.videoId)" :class="{cur: current_id==video.videoId}">
          <td><span>{{video.videoName}}</span><i class="icon-playing" style="margin-left: 5px;"></i></td>
          <!-- <td>未观看</td> -->
...
<script>
  export default {
    name: 'video_table_panel',
    props: ['video_table', 'current_id'],
    computed: {
      // 直播判断
      videos_is_live: function () {
        return this.video_table.typeCode == '08'? true : false
      }
    },
    methods: {
      // 选择视频：
      gotoVideo: function(video_id){
        this.$parent.gotoVideoById(video_id,1)
        $(window).smoothScroll()
      }
    },
    filters: {
